            ...
            if (!resultados) {
                ...
                Inscrito.find({ cedula: cedulaBuscada, idCurso: idCursoBuscado }, (erro, result) => {
                    ...
                    if (erro) {
                        ...
                        console.log('Vamos bien por aca se puede matricular nueva opcion')
...
io.on('connection', client => {
    ...
    client.on('usuarioNuevo', (usuario) => {
        if (usuario == null) {
            usuariosChat.usuarioCurso(client.id, usuario)
        } else {
    ...
    client.on('disconnect', () => {
        if (usuariosChat.getusuarioCurso != "") {

        } else {
            let usuarioBorrado = usuariosChat.borrarUsuario(client.id)
    ...
    client.on('texto', (text, callback) => {
        let usuario = usuariosChat.getUsuario(client.id)
        let texto = `${usuario.nombre}: ${text}`
        io.emit('texto', (texto))
        callback()
    })
    client.on('newCurso', (nuevoCurso) => {
        Usuario.find({rol: 'aspirante'}).exec((err, result) => {
            if (err) {
                return console.log(err)
            } else {              
                for(var i = 0; i < result.length; i++){
                    ...
                    subject: 'Nuevo Curso',
                    html: `Se ha abierto el curso ${nuevoCurso}, si estás interesado 
                ingresa ya a nuestra página para obtener toda la información`
                };
                sgMail.send(msg);                
                }
                console.log('ENTRO PORQUE SE CREO NUEVO CURSO')
                let correo = 'se envio correo'
                io.emit('newCurso', (correo))
            }
        })
    })
});
